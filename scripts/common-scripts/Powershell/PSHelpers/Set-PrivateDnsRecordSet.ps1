...
Function Get-RecordForPrivateDnsZone {
    ...
    foreach ($resourceDomain in $DomainPrivateDnsZoneMappingTable.Keys) {
        ...
        if ($Fqdn.EndsWith('sql.azuresynapse.net')) {
            $foundResourceDomain = 'sql.azuresynapse.net'
            break
        }

        if ($Fqdn.EndsWith('dev.azuresynapse.net')) {
            $foundResourceDomain = 'dev.azuresynapse.net'
            break
        }
...
try {
    ...
    foreach ( $privateEndpointDnsRecord in $privateEndpointDnsRecordObject ) {
        ...
        foreach ($ipAddress in $privateEndpointDnsRecord.IpAddress) {
            ...
            # Update DNS records
            if ($UpdateAllDnsZones) {
                foreach ($region in $regionToResourceGroupMappingTable.Keys) {
                    New-AzPrivateDnsRecordSet `
                        -Name $privateDnsRecord.Name `
                        -ZoneName $privateDnsRecord.Domain `
                        -resourceGroupName $regionToResourceGroupMappingTable[$region] `
                        -Ttl $Ttl `
                        -RecordType 'A' `
